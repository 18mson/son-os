...
export const WindowComponent: React.FC<WindowProps> = ({ window: windowState, children }) => {
  ...
  }, [isDragging, id, moveWindow, size, isMaximized, toggleMaximizeWindow]);
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        height: isMaximized ? "calc(100vh - 72px)" : size.h,
        zIndex: zIndex,
      }}
      className={`flex flex-col rounded-xl overflow-hidden shadow-2xl border transition-shadow duration-200 select-none ${
        isActive
    ...
    >
      ...
      >
        ...
        >
          ...
          <button
            ...
            title="Close"
            className="p-1.5 rounded-lg hover:bg-rose-500/80 hover:text-white transition-colors ml-0.5"
          >
            <X size={15} />
          </button>
        </div>
      ...
      <div className={`flex-1 overflow-auto text-zinc-100 font-sans select-text ${
        id === 'japanese-quiz' || id === 'lovely-ever' ? 'p-0 flex flex-col' : 'p-6'
      }`}>
        {children}
      </div>
    </motion.div>
