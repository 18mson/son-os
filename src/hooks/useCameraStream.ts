      ...
      for (let i = startIndex; i < ladder.length; i++) {
        ...
        try {
          ...
          if (
            ...
            try {
              const expMin = (capabilities as { exposureCompensation?: { min: number; max: number } }).exposureCompensation?.min ?? 0;
              const expMax = (capabilities as { exposureCompensation?: { max: number } }).exposureCompensation?.max ?? 0;
