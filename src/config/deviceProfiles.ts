...
export async function detectDeviceProfile(): Promise<DeviceProfile> {
  ...
  // 1. Coba deteksi modern via Client Hints (navigator.userAgentData)
  if (nav.userAgentData?.getHighEntropyValues) {
    try {
      ...
      // Motorola G45 5G identifiers (e.g. "moto g45 5g", "XT2433", etc.)
      if (model.includes("g45") || model.includes("xt2433") || model.includes("motorola")) {
        return MOTO_G45_5G_PROFILE;
      }
      // Samsung A54 5G identifiers (e.g. "sm-a546b", "sm-a546v", "galaxy a54")
